...
declare const layoutSize: TableLayout;
declare const normalData: RowItemType[];
...
// 左侧冻结列宽
export const leftFixedColumnWidth = computed(() => {
  return sumBy(leftFixedColumns.value, (item) => item.width);
});

// 右侧冻结列宽
export const rightFixedColumnWidth = computed(() => {
  return sumBy(rightFixedColumns.value, (item) => item.width);
});

// 所有列宽
export const allColumnsWidth = computed(() => {
 ...
 */
export const getFixedColumnStyle = (column: TableColumnItem) => {
  if (column.fixed) {
...
// 列偏移
export const getColumnOffset = (column: TableColumnItem): number => {
...
export const findRowIndex = (key: string): number => {
  const { rowKey } = tableOptions;
  return normalData.findIndex((item) => getDataKey(item, rowKey) === key);
};

export const updateColumns = (columns: TableColumnItem[]) => {
